...
# coordinate/callout labels for a video. Called
# from render_static_hud() (baked once per
# video) instead of the live per-frame chain —
# label positions and text never change
# mid-video, so there is no reason to recompute
# them 24 times a second.
#
...
#
# This is the fix for sub-realtime encode speed:
# the old per-frame filter_complex chain ran
# ~55 drawbox/drawtext ops on the full 1280x720
# frame every single frame, which a 2-core
# runner cannot sustain. Nearly all of those ops
# (labels, LIVE badge shell, category chip,
# wordmark, credits text, corner brackets,
# sealed border, ticker plate, ON AIR label,
# channel name) never change mid-video, so
...
#
# Trimmed for CPU budget on a 2-core runner:
# - no vignette (was a full-frame per-pixel
#   pass on every frame for a gaming stream)
# - subscriber/viewer counts no longer drawn
#   (still written to disk by the background
#   pollers if you want to re-enable later)
# - CTA subscribe reminder removed entirely
#   (was 6 draw ops every frame for an 8s/240s
#   payoff)
#############################################
build_dynamic_chain() {
    ...
    # (CTA subscribe reminder intentionally removed here — see comment
    # above build_dynamic_chain.)
...
#############################################
# Up-next bumper — unchanged aside from pinning
# the looped overlay.png to the bumper's own
# output framerate (24fps) instead of the
# image2 demuxer's 25fps default, so there's no
# framesync mismatch to reconcile.
#############################################
run_bumper() {
    local next_url="$1"
...
#
# Main ffmpeg call only needs two inputs — the
# source video and the pre-baked static_hud.png
# — instead of three. overlay.png is dropped
# entirely: nothing in the live filter graph
# referenced it, and its presence at input
# index 1 also meant build_labels_chain's
# "[1:v]" reference was quietly pulling in the
# wrong image instead of the actual dot marker.
# Baking the dot marker and labels into
# static_hud.png ahead of time fixes both
# problems at once.
#############################################
run_video() {
    local url="$1"
